...
unbounded-equality candidate was rejected because it required another linear
pass when a large mapping changed near the end. The constant-size check is
independent of scene shape. Regression tests cover old snapshots, version links,
exact slots, permutations, key additions/removals, duplicates, empty transitions,
and removal patches.
