//! Same independent WGSL/inputs used by the Windows probes, forced to Metal.
...
impl Reference {
    pub fn new(identity: u64) -> Result<Self> {
        ...
        for adapter in pollster::block_on(instance.enumerate_adapters(backend)) {
            ...
            // SAFETY: read-only registry identity; the raw device does not escape the guard.
            let registry_id = unsafe { device.as_hal::<wgpu::hal::api::Metal>() }
                .ok_or("wgpu did not create Metal")?
                .raw_device()
                .registryID();
            if registry_id != identity {
                continue;
            }
            let mut entries = [
            ...
            let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
                ...
                source: wgpu::ShaderSource::Wgsl(
                    include_str!("../../src/native/runtime/tests/reference.wgsl").into(),
                ),
            });
        ...
        Err("same physical Metal GPU unavailable in wgpu".into())
    }
