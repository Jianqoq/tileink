...
#[cfg(test)]
pub(crate) use crate::cubecl::brush::GPU_BRUSH_U32_STRIDE;

const INVALID_REF: u32 = u32::MAX;

/// CPU-side mirror of the draw fields uploaded to CubeCL buffers.
///
/// The semantic scene remains record-based for CPU rendering and editing, while
/// this cache stores CubeCL's columnar layout at mutation time. Render-time full
/// uploads can borrow these slices directly, and incremental paths can patch the
/// same slots without walking every draw record again.
