...
float noise_gradient(const device float2* gradients,uint table,uint channel,uint selector,float2 p,float rounding_zero) {
    float2 gradient=gradients[table*2056+channel*514+selector];
    return fma(gradient.x,p.x,fma(gradient.y,p.y,rounding_zero));
}
float perlin(const device uint* selectors,const device float2* gradients,uint table,uint channel,float2 position,bool stitch,int2 wrap,int2 extent,float rounding_zero) {
    ...
    float v00=noise_gradient(gradients,table,channel,table_data[left+lower.y],p,rounding_zero);
    float v10=noise_gradient(gradients,table,channel,table_data[right+lower.y],p-float2(1,0),rounding_zero);
    float v01=noise_gradient(gradients,table,channel,table_data[left+upper.y],p-float2(0,1),rounding_zero);
    float v11=noise_gradient(gradients,table,channel,table_data[right+upper.y],p-1.0f,rounding_zero);
