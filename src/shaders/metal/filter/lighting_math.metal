float light_dot(float3 a,float3 b) {return fma(a.x,b.x,fma(a.y,b.y,fma(a.z,b.z,0.0f)));}
...
uint lighting_pixel(constant FilterConfig& c,texture2d<float,access::read> source,uint2 xy) {
    ...
    float normal_length=sqrt(light_dot(normal,normal));
    ...
    if(c.light_kind==0) {
        ...
        float length=sqrt(light_dot(light,light));if(length<=0.000001f) return dark;
        light/=length;
        if(c.light_kind==2) {
            float3 spot(c.light_p3-c.light_p0,c.light_p4-c.light_p1,c.light_p5-c.light_p2);
            float length=sqrt(light_dot(spot,spot));if(length<=0.000001f) return dark;
            spot/=length;
            float focus=-light_dot(light,spot);
    ...
    if(c.lighting_output_kind==0) {
        float intensity=c.light_constant*attenuation*max(light_dot(normal/normal_length,light),0.0f);
        return pack_pixel(float4(clamp(color*intensity,0.0f,1.0f),1));
    }
    float3 halfway(light.xy,light.z+1.0f);float length=sqrt(light_dot(halfway,halfway));
    if(length<=0.000001f) return dark;
    float cosine=max(light_dot(normal,halfway)/(normal_length*length),0.0f);
