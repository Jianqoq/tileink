#ifndef TILEINK_HLSL_SHARED_DRAW_HLSLI_INCLUDED
#define TILEINK_HLSL_SHARED_DRAW_HLSLI_INCLUDED
...
DrawData load_draw(ByteAddressBuffer records, uint index) {
...
bool draw_has_sdf(DrawData draw) { return draw.sdf_offset != INVALID_INDEX || draw.shadow_offset != INVALID_INDEX; }
...
bool draw_hits_tile(DrawData draw, uint2 tile, uint2 dimensions) {
...
uint draw_backdrop(ByteAddressBuffer paths, DrawData draw, uint2 tile, uint2 dimensions) {
...
float2 draw_point(DrawData draw, float2 position) {
...
bool transformed_rect_hits_tile(DrawData draw, int4 rect, uint2 tile) {
...
#endif // TILEINK_HLSL_SHARED_DRAW_HLSLI_INCLUDED
