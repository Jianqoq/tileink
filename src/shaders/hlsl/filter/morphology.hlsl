...
void filter_morphology_axis_region(uint3 gid:SV_DispatchThreadID) {
    uint2 xy;
    if (filter_position(config,active_tiles,gid,xy))
        target_texture[xy]=rgba8_to_unorm(filter_morphology_pixel(config,source_texture,xy));
}
