...
ByteAddressBuffer convolve_kernels : register(t6);
ByteAddressBuffer active_tiles : register(t8);
[numthreads(FILTER_WORKGROUP_SIZE,1,1)]
void filter_convolve_matrix_region(uint3 gid:SV_DispatchThreadID) {
    uint2 xy;
    if (filter_position(config,active_tiles,gid,xy))
        target_texture[xy]=rgba8_to_unorm(filter_convolve_pixel(config,source_texture,convolve_kernels,xy));
}
