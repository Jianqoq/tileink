...
fn filter_source_sample_premul(x: f32, y: f32) -> vec4<f32> {
    ...
    let uv = (position + vec2<f32>(0.5)) / vec2<f32>(textureDimensions(source_texture));
    return textureSampleLevel(source_texture, filter_linear_sampler, uv, 0.0);
}

fn filter_aux_sample_premul(x: f32, y: f32) -> vec4<f32> {
    let position = clamp(vec2<f32>(x, y), vec2<f32>(0.0), vec2<f32>(f32(config.width - 1u), f32(config.height - 1u)));
    let uv = (position + vec2<f32>(0.5)) / vec2<f32>(textureDimensions(aux_texture));
    return textureSampleLevel(aux_texture, filter_linear_sampler, uv, 0.0);
}
