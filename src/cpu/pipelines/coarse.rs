...
impl<'a> CoarseCpuPrepared<'a> {
    pub fn run(&mut self) {
        for draw in self.draw_records {
            ...
            self.rasterize_draw(draw, backdrop_record, bbox);
        }
    }

    fn rasterize_draw(&mut self, draw: &DrawRecord, backdrop_record: &BackdropRecord, bbox: TileBbox) {
        ...
        for tile_y in bbox.y0..bbox.y1 {
            for tile_x in bbox.x0..bbox.x1 {
                ...
                let segments = &self.segments[segment_range.start as usize..segment_range.end as usize];
