...
impl Canvas {
    ...
    fn append_scene_data(&mut self, other: &Canvas, offset: SceneOffset) -> usize {
        ...
        for draw in &other.draw_records {
            ...
            if let Some(sdf_id) = &mut draw.sdf_id {
                *sdf_id = sdf_id.saturating_add(sdf_offset);
                draw.pixel_bounds =
                    Self::pixel_bounds_from_bounds(self.sdfs[*sdf_id as usize].bounds());
            } else if let Some(sdf_shadow_id) = &mut draw.sdf_shadow_id {
                *sdf_shadow_id = sdf_shadow_id.saturating_add(sdf_shadow_offset);
                draw.pixel_bounds = Self::pixel_bounds_from_bounds(
                    self.sdf_shadows[*sdf_shadow_id as usize].bounds(),
                );
            }
            if !offset.is_zero() {
    ...
    fn push_sdf_shadow_record(
        ...
        let draw_ix = self.push_draw_record(DrawRecord {
            ...
            glyph_run_id: None,
            sdf_id: None,
            sdf_shadow_id: Some(sdf_shadow_id),
            brush_id,
            tag,
            fill_rule: SDF_RECORD_FILL_RULE,
            pixel_bounds: PixelBounds {
