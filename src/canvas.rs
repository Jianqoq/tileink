...
pub(crate) struct RetainedSceneCache {
    scenes: HashMap<RetainedSceneCacheId, SharedArc<Canvas>>,
}
...
impl Canvas {
    ...
    /// retained scene cache keyed by `(element id, slot, revision, position)`. This is not a
    /// texture cache: retained scenes still render through the regular vector path and preserve
    /// exact quality.
