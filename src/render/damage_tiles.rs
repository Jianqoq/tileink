...
impl DamageTiles {
    ...
    /// Adds vertical sampling halos without filling gaps between sparse columns.
    pub(crate) fn outset_y(&self, pixels: i32) -> Self {
        if pixels <= 0 {
            return self.clone();
        }
        let rows = (pixels as u32).div_ceil(TILE_SIZE);
        ...
        for &tile in &self.list {
            let x = tile % self.tiles_width;
            let y = tile / self.tiles_width;
            let end = y
                .saturating_add(rows)
                .saturating_add(1)
                .min(self.tiles_height);
            for row in y.saturating_sub(rows)..end {
                expanded.add_tile(row * self.tiles_width + x);
            }
        }
        expanded
    }
...
pub(crate) fn tile_count_for_bounds(bounds: Bounds) -> u32 {
    if bounds.is_empty() {
        return 0;
    }
