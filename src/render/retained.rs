//! Per-renderer retained preparation, damage, history, and surface-cache decisions.
//!
//! GPU allocations are opaque owners; their API and retirement remain in the adapter.
...
impl<T: SurfaceAllocation> RetainedRenderState<T> {
    ...
    pub(crate) fn config(&self) -> IncrementalRenderConfig {
        self.config
    }

    pub(crate) fn set_config(&mut self, config: IncrementalRenderConfig) {
    ...
    ) -> crate::render::incremental::IncrementalRenderMode {
        std::mem::replace(&mut self.config.mode, mode)
    }

    pub(crate) fn stats(&self) -> &IncrementalRenderStats {
        &self.stats
    }

    pub(crate) fn stats_mut(&mut self) -> &mut IncrementalRenderStats {
        &mut self.stats
    }

    pub(crate) fn active_tiles(&self) -> Option<&DamageTiles> {
    ...
    pub(crate) fn bypasses_backdrop_cache(&self) -> bool {
        matches!(
    ...
    pub(crate) fn set_active_tiles(&mut self, active: Option<DamageTiles>) {
    ...
    pub(crate) fn set_history_owner(&mut self, owner: HistoryOwner) {
    ...
    pub(crate) fn select_materialized(
        &mut self,
        scene: Rc<Canvas>,
    ...
    pub(crate) fn begin_frame(
        ...
        self.surfaces.begin_frame(!plan.stats.full_redraw);
        plan
    }

    pub(crate) fn finish_frame(&mut self, plan: DamagePlan, rendered: bool, history_updated: bool) {
        self.surfaces.end_frame();
    ...
    pub(crate) fn surface_is_dirty(&self, bounds: Bounds) -> bool {
        self.active_tiles
            .as_ref()
            .is_none_or(|tiles| tiles.intersects_bounds(bounds))
    }

    pub(crate) fn local_damage_for_surface(
        &self,
        surface: Bounds,
        root_size: (u32, u32),
    ) -> Option<DamageTiles> {
    ...
    pub(crate) fn take_matching_surface(
        &mut self,
        id: Option<RetainedSurfaceId>,
        meta: Option<RetainedSurfaceMeta>,
    ) -> Option<(RetainedSurfaceId, RetainedSurface<T>)> {
        let id = id?;
    ...
    pub(crate) fn insert_surface(&mut self, id: RetainedSurfaceId, surface: RetainedSurface<T>) {
    ...
    pub(crate) fn backdrop_is_dirty(&self, id: Option<RetainedSurfaceId>) -> bool {
...
pub(crate) enum HistoryOwner {
    #[default]
    Internal,
    External(ExternalTextureHistoryId),
}

pub(crate) enum SelectedScene<'a> {
    Borrowed(&'a Canvas),
    Retained {
        scene: Rc<Canvas>,
        frame: RetainedFrame,
...
impl SelectedScene<'_> {
    pub(crate) fn scene(&self) -> &Canvas {
        match self {
    ...
    pub(crate) fn frame(&self) -> Option<RetainedFrame> {
        match self {
    ...
    pub(crate) fn materialized_reused(&self) -> bool {
        matches!(
            self,
            Self::Retained {
