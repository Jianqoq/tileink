...
#[test]
fn vertical_halos_match_pixel_bounds_and_preserve_sparse_columns() {
    for size in [(0, 0), (1, 1), (17, 19), (129, 97)] {
        let mut damage = DamageTiles::new(size);
        for y in (0..size.1).step_by(32) {
            for x in (0..size.0).step_by(48) {
                damage.add_bounds(Bounds::new(x as i32, y as i32, x as i32 + 1, y as i32 + 1));
            }
        }
        for pixels in [-1_i32, 0, 1, 15, 16, 17, 32, i32::MAX] {
            let mut expected = DamageTiles::new(size);
            for rect in damage.coalesced_rects(size) {
                expected.add_bounds(
                    Bounds::new(
                        rect.x0,
                        rect.y0.saturating_sub(pixels.max(0)),
                        rect.x1,
                        rect.y1.saturating_add(pixels.max(0)),
                    )
                    .intersect(Bounds::canvas(size.0, size.1)),
                );
            }
            let mut actual = damage.outset_y(pixels).list().to_vec();
            let mut expected = expected.list().to_vec();
            actual.sort_unstable();
            expected.sort_unstable();
            assert_eq!(actual, expected, "size={size:?}, pixels={pixels}");
        }
    }
}
