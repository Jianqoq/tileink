...
pub fn generate() -> io::Result<()> {
    ...
    if !targets.is_empty() {
        ...
        let graph = SourceGraph::load(&root.join("src/shaders/hlsl"), "probes.hlsl")?;
        ...
        let abi = fs::read(root.join("src/shaders/probe-abi.json"))?;
        let description: serde_json::Value = serde_json::from_slice(&abi)?;
        abi::validate(&description)?;
        let programs = description["programs"]
            .as_array()
            .ok_or_else(|| io::Error::other("missing native probe program inventory"))?;
        ...
        for target in targets {
            for program in programs {
                ...
                let recipe = serde_json::json!({"schema":1,"language":"hlsl","target":target,"target_triple":target_triple,
                    "entry":entry,"variant":"probe","flags":flags,"abi_sha256":digest(&abi),
                ...
                declarations.push_str(&format!("NativeShaderArtifact {{ format: {target:?}, entry: {entry:?}, cache_key: {:?}, bytes: include_bytes!({:?}) }},\n",key.hex(),output));
