...
#[cfg(test)]
#[path = "adapter/swapchain_tests.rs"]
mod swapchain_tests;
...
pub struct PhysicalDeviceFeatures {
    ...
    /// Queried only when the instance and device maintenance extensions are available.
    swapchain_maintenance1: Option<vk::PhysicalDeviceSwapchainMaintenance1FeaturesEXT<'static>>,
}

fn swapchain_maintenance_feature(
    supported: Option<&vk::PhysicalDeviceSwapchainMaintenance1FeaturesEXT<'_>>,
    enabled_extensions: &[&CStr],
) -> Option<vk::PhysicalDeviceSwapchainMaintenance1FeaturesEXT<'static>> {
    (enabled_extensions.contains(&ext::swapchain_maintenance1::NAME)
        && supported.is_some_and(|feature| feature.swapchain_maintenance1 == vk::TRUE))
    .then(|| {
...
impl PhysicalDeviceFeatures {
    ...
    fn from_extensions_and_requested_features(
        ...
        Self {
            swapchain_maintenance1: swapchain_maintenance_feature(
                phd_features.swapchain_maintenance1.as_ref(),
                enabled_extensions,
            ),
...
impl super::InstanceShared {
    fn inspect(
        ...
        features.core = if let Some(ref get_device_properties) = self.get_physical_device_properties
        {
            ...
            if self.extensions.contains(&ext::surface_maintenance1::NAME)
                && capabilities.supports_extension(ext::swapchain_maintenance1::NAME)
            {
                let next = features.swapchain_maintenance1.insert(Default::default());
                features2 = features2.push_next(next);
            }
...
impl super::Adapter {
    ...
    pub fn required_device_extensions(&self, features: wgt::Features) -> Vec<&'static CStr> {
        ...
        if self
            ...
            .is_some_and(|feature| feature.swapchain_maintenance1 == vk::TRUE)
        {
    ...
    pub unsafe fn device_from_raw(
        ...
        let shared = Arc::new(super::DeviceShared {
            raw: raw_device,
            swapchain_maintenance1: swapchain_maintenance_feature(
                self.phd_features.swapchain_maintenance1.as_ref(),
                enabled_extensions,
            )
            .is_some(),
            family_index,
