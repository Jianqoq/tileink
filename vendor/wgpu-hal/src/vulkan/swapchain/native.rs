...
pub(crate) struct NativeSurface {
    ...
    format_capacity: core::sync::atomic::AtomicU32,
...
impl NativeSurface {
    pub fn from_vk_surface_khr(
        ...
        Self {
            ...
            format_capacity: core::sync::atomic::AtomicU32::new(0),
            #[cfg(windows)]
...
impl Surface for NativeSurface {
    fn surface_capabilities(
        ...
        let raw_surface_formats = {
            ...
            match super::surface_formats::query_surface_formats(
                &self.format_capacity,
                |count, output| unsafe {
                    (self.functor.fp().get_physical_device_surface_formats_khr)(
                        adapter.raw,
                        self.raw,
                        count,
                        output.map_or(core::ptr::null_mut(), |formats| formats.as_mut_ptr()),
                    )
                },
            ) {
                Ok(formats) => formats,
    ...
    unsafe fn create_swapchain(
        ...
        if device.shared.swapchain_maintenance1 {
            // Resize often retires images before they are ever acquired. Defer their memory
            // allocation to the first acquire, avoiding work for those unused images. This
            // addresses repeated eager allocation while retaining driver synchronization.
            raw_flags |= vk::SwapchainCreateFlagsKHR::DEFERRED_MEMORY_ALLOCATION_EXT;
        }
