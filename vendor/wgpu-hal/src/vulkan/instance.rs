...
fn add_surface_maintenance_extensions(
    extensions: &mut Vec<&'static CStr>,
    supports: impl Fn(&CStr) -> bool,
) {
    if extensions.contains(&khr::surface::NAME)
        && supports(khr::get_surface_capabilities2::NAME)
        && supports(ext::surface_maintenance1::NAME)
    {
        extensions.extend([
...
#[cfg(test)]
#[path = "instance/surface_maintenance_tests.rs"]
...
impl super::Instance {
    ...
    pub fn desired_extensions(
        ...
        add_surface_maintenance_extensions(&mut extensions, |name| {
            instance_extensions
                .iter()
