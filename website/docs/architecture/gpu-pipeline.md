...
Windows ?? DX12/Vulkan retained ???? materializer?damage?journal ???
filter/offscreen ???????????? GPU buffer ??? dirty ranges?????
?? GPU ???????????? retained history ????????

???? `native_interop` ?? device/queue/texture?????? `NativeTargetUse`
??????? incoming/outgoing state?DX12 ?? fence/value?Vulkan ????
binary/timeline semaphore??? stage/access ? queue family?Tileink ?????
acquire/release barrier???????????? barrier ? acquire/present?
? damage ???????????????????????????

`native_present` ???? API ??????resize ? GPU present?DX12 ?? UAV
????? GPU copy?Vulkan ??????? swapchain image????? CPU ??
??????????????? context/renderer?gfx_ui feature ????????
